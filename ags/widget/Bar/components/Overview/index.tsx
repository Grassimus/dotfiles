      ...
      >
        <Gtk.DropTarget
          ...
          onDrop={(_, value) => {
            if (value) {
              hypr.message_async(
                `dispatch movetoworkspacesilent ${ws.get_id()},address:0x${value}`,
                null,
              )
            }
            return true
  ...
  export function Button() {
    ...
    return (
      <PanelButton
        name="overview"
        class="workspaces"
        onClicked={() => toggleWindow("overview")}
      >
        <box valign={CENTER}>
          <For each={workspaces}>
            {(ws) => {
              const className = createBinding(hypr, "focusedWorkspace").as(
                (fws) => {
    ...
    const workspaces = createComputed(() =>
      dummyWorkspaces(
        hyprWorkspaces()
          .filter((w) => w.id !== -99)
          .sort((a, b) => a.id - b.id),
        options.overview.workspaces(),
      ),
    )

    return (
      <PopupWindow application={app} name="overview" exclusive layer={OVERLAY}>
